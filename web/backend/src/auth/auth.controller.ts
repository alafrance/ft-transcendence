import {Body, Controller, Get, Query, Res, Req, Param, Post, UseGuards} from '@nestjs/common';
...
export class AuthController {
	...
	async login(@Req() req, @Res() res) {
		await this.authService.createCookieById(req.user.id, res);
	...
	async register(@Body() body, @Res() res) {
		...
		if (newUser.error || newUser == undefined || !newUser.id)
			return res.send(newUser);
		await this.authService.createCookieById(newUser.id, res);
