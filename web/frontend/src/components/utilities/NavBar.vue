...
<!--				<a  :href="this.$store.state.REDIRECT_URI">-->
        <router-link :to="{name: 'login'}">
            ...
            <font-awesome-icon v-if="!user.avatar" icon="user" class="w-12 h-12 color-accent" />
						<AvatarUtility v-if="user.avatar" :avatar-link="user.avatar" class="inline-flex" :size="16" />
					</MenuButton>
				</div>
                ...
                <button @click="logout" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm w-full flex flex-start']">
